...
def load_multi_dict():
    gene_file = "Dataset/hic_features/hic_multi_nor.txt"
    Data = pd.read_table(gene_file, sep='\t', header='infer', names=None, index_col=None, dtype=None, engine=None,
                         nrows=None)
    return Data

def main():
    path = "Dataset/cell_cycle.txt"
    cell_inf = pd.read_table(path, sep='\t', header='infer', names=None, index_col=None, dtype=None, engine=None,
                             nrows=None)
    # cell_inf 是一个dataframe (6288,2)
    ...
    for value in num.values():
        ...
        alpha.append(ds)
    print(alpha)
    rldcp_feature = load_rldcp_dict()  # 6288 * 3001  3000个特征
    ...
    for seed in test_seed:
        ...
        test_acc, test_label, real_label, test_result_matrix = CNN_1D_network(rldcp_feature,gene_feature,multi_feature, X_train, y_train, X_test,
