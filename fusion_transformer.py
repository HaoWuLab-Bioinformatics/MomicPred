...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, key_size, query_size, value_size, num_hiddens,
        ...
        # ffn_num_input, ffn_num_hiddens, ffn_num_outputs
...
class SE_block(nn.Module):
    def __init__(self, channel=2666, scaling=16): #scaling为缩放比例，
                                           # 用来控制两个全连接层中间神经网络神经元的个数，一般设置为16，具体可以根据需要微调
        super(SE_block, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool1d(2666)
    ...
    def forward(self, x):
        b, c, n = x.size()
        y = self.avg_pool(x)
        # print(y.size()) # 32*1*2660
        y = self.fc(y)
        # print(y.size())
        return x * y

class ECA_block(nn.Module):
    def __init__(self, channel=1, b=1, gamma=2):
    ...
    def forward(self, x):
        ...
        out = x * y.expand_as(x)
        return out
class ECA_block_para(nn.Module):
    def __init__(self, channel, b, gamma): # 2 1 4
        super(ECA_block_para, self).__init__()
        kernel_size = int(abs((math.log(channel, 2) + b) / gamma))
        kernel_size = kernel_size if kernel_size % 2 else kernel_size + 1
