...
def main():
    folder_path = './Data_Preparation/GSE_process1'  # 替换为你的文件夹路径
    files = os.listdir(folder_path)
    for file in files:
        mkdir('./Data_Preparation/GSE_process2/' + file.split('.')[0].split('_')[1])
        # print(file.split('.')[0].split('_')[1])  # GaseE752226
        path = './Data_Preparation/GSE_process1' + '/' + file
        f = open(path)
        a = []
        count = 1
        for line in f.readlines():
            if count >= 25:
                a.append(line.split())
            count += 1
        write_reads(out_path='./Data_Preparation/GSE_process2/' + file.split('.')[0].split('_')[1], chr_pair_list=a)
    print(files)
