...
def main():
    file_list = np.loadtxt('./Data_Preparation/cell_cycle.txt', dtype=str)   #
    need_cell = []  # 选出需要的细胞
    for file in file_list:
        need_cell.append(file[0])
    # print(need_cell)
    folder_path = './Data_Preparation/GSE223917'  # 解压出来的数据文件夹
    ...
    for cell in files:
        ...
        if contains_substring(cell.split('_')[1].split('.')[0], need_cell):
            ...
            destination_folder = './Data_Preparation/GSE_process1'  # 替换为你的目标文件夹路径
