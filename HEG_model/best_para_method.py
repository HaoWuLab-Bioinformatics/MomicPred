...
def CNN_train(epoch, model, optimizer, train_loader, loss_fn, device):
    i = 0
    RLDCP_loader, gene_loader = train_loader
    for  (images_RLDCP, labels_RLDCP), (images_gene, labels_gene)in zip(RLDCP_loader, gene_loader):
        optimizer.zero_grad()
        ...
        outputs = model(images_RLDCP, images_gene)
...
def CNN_val(epoch, model, test_loader, loss_fn, test_size, device):
    RLDCP_loader, gene_loader= test_loader
    i = 0
    for  (images_RLDCP, labels_RLDCP), (images_gene, labels_gene)in zip(RLDCP_loader, gene_loader):
        images_RLDCP = torch.unsqueeze(images_RLDCP.type(torch.FloatTensor), dim=1)
        images_RLDCP = images_RLDCP.to(device)
...
class create_model(nn.Module):
    def __init__(self, model_para,transformer_para,eca_para,lstm_para):
        ...
        linear_size_RLDCP_init = 2660
        linear_size_gene_init = 300
        ...
        if linear_layer == 1:
            ...
            self.linear_RLDCP.add_module("linear1", nn.Linear(
                in_features=  (cnn_feature * linear_size_gene ) +linear_size_gene_init*2,
                 out_features=( cnn_feature * linear_size_gene + linear_size_gene_init*2) // 2))
            ...
            self.linear_RLDCP.add_module("linear2", nn.Linear(
                in_features=(( + (cnn_feature * linear_size_gene ) +linear_size_gene_init*2) // 2),
                                                               out_features = 4))
...
def CNN_1D_network(RLDCP,gene_feature,multi_feature, tr_x, tr_y, X_test, y_test, lr, model_para, alpha, gamma,transformer_para,eca_para,lstm_para):
    ...
    model.to(device)
    print(model)
    train_loader = [train_loader_RLDCP, train_loader_gene]
    test_loader = [test_loader_RLDCP, test_loader_gene]
    num_epochs = 40
    min_loss = 100000.0
